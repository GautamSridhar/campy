...
def AutoParams(params, default_params):
    ...
    for i in range(len(range_params)):
        ...
        if params[key] <= 0:
            ...
            print(
                "{} set to invalid value in config. Setting to default ({}).".format(
                    key, default_value
                )
            )
...
def ConfigureParams():
    parser = ArgumentParser(
        description="Campy CLI", formatter_class=ArgumentDefaultsHelpFormatter,
    )
    ...
    for key in cam_params:
        if type(cam_params[key]) is list:
            if len(cam_params[key]) == cam_params["numCams"]:
                ...
                logging.warning(
                    "{} size mismatch with numCams. Using list idx {}.".format(
                        key, cam_params["n_cam"]
                    )
                )
                cam_params[key] = cam_params[key][cam_params["n_cam"]]
    return cam_params
...
def LoadConfig(config_path):
    try:
        ...
        logging.error(
            "Caught this error at configurator.py LoadConfig: {}. Check your config path!".format(
                e
            )
        )
        raise
...
def ParseClargs(parser):
    ...
    parser.add_argument(
        "--videoFilename", dest="videoFilename", help="Name for video output file.",
    )
    parser.add_argument(
        "--frameRate",
        dest="frameRate",
    ...
    parser.add_argument(
        "--frameHeight", dest="frameHeight", type=int, help="Frame height in pixels.",
    )
    parser.add_argument(
        "--frameWidth", dest="frameWidth", type=int, help="Frame width in pixels.",
    )
    parser.add_argument(
        "--cameraDebug",
        dest="cameraDebug",
